...
export default function ProfesorApp() {
  ...
  // ── Login — llama buscarProfesor directamente al entrar ─
  async function login() {
    ...
    // Busca el profesor inmediatamente sin esperar onAuthStateChange
    if (data.user?.email) {
  ...
  async function marcarDada() {
    ...
    const { data: contrato } = await supabase
      ...
      .single()
    const fraccion = contrato ? claseActiva.duracion_min / contrato.duracion_min : 1
  ...
  if (!sesion) return (
    <div style={{ position: 'fixed', inset: 0, background: `linear-gradient(150deg, ${TEAL} 0%, #0d5f5f 100%)`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '24px', boxSizing: 'border-box' }}>
      <style>{`
        ...
        .li:focus { border-color:${TEAL} !important; box-shadow:0 0 0 3px ${TEAL}33 !important; outline:none !important; }
      `}</style>
      <div style={{ width: '100%', maxWidth: '360px', animation: 'fadeUp 0.4s ease' }}>
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          <div style={{ width: '88px', height: '88px', background: 'rgba(255,255,255,0.15)', borderRadius: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', fontSize: '40px', border: '1px solid rgba(255,255,255,0.2)' }}>🎵</div>
        ...
        <div style={{ background: 'white', borderRadius: '24px', padding: '32px', boxShadow: '0 24px 80px rgba(0,0,0,0.3)' }}>
          <div style={{ marginBottom: '18px' }}>
            ...
            <input className="li" type="email" value={loginEmail} onChange={e => setLoginEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && login()}
              style={{ width: '100%', padding: '14px 16px', border: '2px solid #e5e7eb', borderRadius: '12px', fontSize: '15px', boxSizing: 'border-box', transition: 'border-color 0.2s', fontFamily: 'inherit' }} placeholder="nombre@email.com" />
          </div>
          <div style={{ marginBottom: '24px' }}>
            ...
            <input className="li" type="password" value={loginPass} onChange={e => setLoginPass(e.target.value)} onKeyDown={e => e.key === 'Enter' && login()}
              style={{ width: '100%', padding: '14px 16px', border: '2px solid #e5e7eb', borderRadius: '12px', fontSize: '15px', boxSizing: 'border-box', transition: 'border-color 0.2s', fontFamily: 'inherit' }} placeholder="••••••••" />
          </div>
          {loginError && (
            <div style={{ background: '#fef2f2', border: '1px solid #fecaca', borderRadius: '10px', padding: '11px 14px', marginBottom: '18px', color: '#dc2626', fontSize: '13px', fontWeight: '600', textAlign: 'center' }}>{loginError}</div>
          )}
  ...
  return (
    // ── Wrapper externo cubre TODA la pantalla ────────────
    <div style={{ position: 'fixed', inset: 0, background: '#f8fafc', display: 'flex', justifyContent: 'center' }}>
      ...
      <div style={{ width: '100%', maxWidth: '480px', height: '100%', display: 'flex', flexDirection: 'column', background: '#f8fafc' }}>
        ...
        <div style={{ background: TEAL, padding: '20px 20px 0', flexShrink: 0 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '18px' }}>
            ...
            <button onClick={() => supabase.auth.signOut()}
              style={{ background: 'rgba(255,255,255,0.15)', border: 'none', color: 'rgba(255,255,255,0.85)', padding: '8px 14px', borderRadius: '20px', cursor: 'pointer', fontSize: '12px', fontWeight: '700', marginTop: '6px', fontFamily: 'inherit' }}>
              Salir
            </button>
          </div>
          <div style={{ display: 'flex', gap: '4px' }}>
            {(['hoy', 'historial'] as const).map(v => (
              <button key={v} onClick={() => setVista(v)}
                style={{ flex: 1, padding: '12px 8px', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '700', borderRadius: '14px 14px 0 0', background: vista === v ? 'white' : 'transparent', color: vista === v ? TEAL : 'rgba(255,255,255,0.65)', transition: 'all 0.2s', fontFamily: 'inherit' }}>
                {v === 'hoy' ? '📅  Hoy' : '📋  Historial'}
              </button>
            ))}
          </div>
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '16px' }}>
          ...
          {vista === 'hoy' && (
            <div style={{ animation: 'fadeUp 0.3s ease' }}>
              ...
              {cargandoClases && <p style={{ textAlign: 'center', color: '#9ca3af', fontSize: '14px', padding: '50px 0' }}>Cargando...</p>}
              ...
              {clases.map((c, i) => {
                ...
                return (
                  <div key={c.id} className="tc" onClick={() => abrirModal(c)}
                    style={{ background: 'white', borderRadius: '18px', padding: '16px', marginBottom: '12px', boxShadow: '0 2px 12px rgba(0,0,0,0.06)', borderLeft: `4px solid ${confirmada ? TEAL : '#e5e7eb'}`, animation: `fadeUp ${0.15 + i * 0.06}s ease`, cursor: 'pointer' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '12px' }}>
                      ...
                      <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        <span style={{ padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: '700', background: badge.bg, color: badge.color, display: 'block', marginBottom: '8px', whiteSpace: 'nowrap' }}>{badge.label}</span>
                        {confirmada && <span style={{ fontSize: '11px', color: TEAL, fontWeight: '700' }}>Toca →</span>}
                      </div>
                    </div>
                    {c.observaciones && (
          ...
          {vista === 'historial' && (
            <div style={{ animation: 'fadeUp 0.3s ease' }}>
              ...
              {cargandoClases && <p style={{ textAlign: 'center', color: '#9ca3af', fontSize: '14px', padding: '50px 0' }}>Cargando...</p>}
              {!cargandoClases && clases.length === 0 && <p style={{ textAlign: 'center', color: '#9ca3af', fontSize: '13px', padding: '40px 0' }}>Sin clases este mes</p>}
              {clases.map((c, i) => {
                const badge = badgeEstado(c.estado, c.revision_pendiente)
                return (
                  <div key={c.id} className="tc" onClick={() => abrirModal(c)}
                    style={{ background: 'white', borderRadius: '14px', padding: '14px 16px', marginBottom: '8px', boxShadow: '0 1px 6px rgba(0,0,0,0.05)', animation: `fadeUp ${0.1 + i * 0.03}s ease`, cursor: 'pointer' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px' }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '5px', flexWrap: 'wrap' }}>
                          ...
                          <span style={{ padding: '2px 9px', borderRadius: '20px', fontSize: '11px', fontWeight: '700', background: badge.bg, color: badge.color, flexShrink: 0 }}>{badge.label}</span>
                        </div>
                        ...
                        {c.observaciones && (
                          <p style={{ margin: '4px 0 0', fontSize: '12px', color: '#6b7280', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>📝 {c.observaciones}</p>
                        )}
                      </div>
      ...
      {claseActiva && (
        <div onClick={e => e.target === e.currentTarget && cerrarModal()}
          ...
          <div style={{ width: '100%', maxWidth: '480px', background: 'white', borderRadius: '28px 28px 0 0', padding: '20px 20px 36px', animation: 'slideUp 0.3s cubic-bezier(0.34,1.56,0.64,1)', maxHeight: '88vh', overflow: 'auto' }}>
            ...
            {claseActiva.revision_pendiente && (
              <div style={{ background: '#fff7ed', border: '1px solid #fed7aa', borderRadius: '12px', padding: '11px 14px', marginBottom: '18px', fontSize: '13px', color: '#c2410c', fontWeight: '600' }}>
                ⚠️ Inasistencia registrada — pendiente de revisión
              </div>
            )}
            <div style={{ marginBottom: '20px' }}>
              ...
              <textarea value={resumen} onChange={e => setResumen(e.target.value)}
                ...
                rows={3}
                style={{ width: '100%', padding: '13px 14px', border: `2px solid ${TEAL_MID}`, borderRadius: '14px', fontSize: '14px', resize: 'none', boxSizing: 'border-box', fontFamily: 'system-ui,-apple-system,sans-serif', lineHeight: '1.6', transition: 'border-color 0.2s', color: '#1f2937' }} />
            </div>
            {claseActiva.estado === 'confirmada' && !claseActiva.revision_pendiente && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '12px' }}>
                <button className="ba" onClick={marcarDada} disabled={guardando}
                  style={{ padding: '16px 12px', background: TEAL, color: 'white', border: 'none', borderRadius: '16px', fontSize: '16px', fontWeight: '800', cursor: guardando ? 'not-allowed' : 'pointer', opacity: guardando ? 0.7 : 1, fontFamily: 'inherit' }}>
                  ✓ Clase dada
                </button>
                <button className="ba" onClick={marcarInasistencia} disabled={guardando}
                  style={{ padding: '16px 12px', background: '#fff7ed', color: '#c2410c', border: '2px solid #fed7aa', borderRadius: '16px', fontSize: '16px', fontWeight: '800', cursor: guardando ? 'not-allowed' : 'pointer', opacity: guardando ? 0.7 : 1, fontFamily: 'inherit' }}>
                  ✗ No asistió
                </button>
              </div>
            )}
