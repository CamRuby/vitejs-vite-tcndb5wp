...
export default function Profesores() {
  ...
  async function cargarClases(p: any) {
    ...
    const { data } = await supabase
      .from('clases')
      .select('id, fecha, hora, duracion_min, estado, revision_pendiente, observaciones, honorario_valor, modalidad, contratos(clientes(nombre), instrumentos(nombre)), salones(nombre, sedes(nombre))')
      .eq('profesor_id', p.id)
      .gte('fecha', fi)
      .lte('fecha', ff)
  ...
  function getHon(c: any) {
    ...
    let t = tarifas.find(x => x.modalidad?.toLowerCase() === modalidad && x.duracion_min === c.duracion_min && !x.taller_grupal)
    if (!t && (modalidad === 'presencial' || modalidad === 'virtual')) {
      t = tarifas.find(x => (x.modalidad?.toLowerCase() === 'presencial' || x.modalidad?.toLowerCase() === 'virtual') && x.duracion_min === c.duracion_min && !x.taller_grupal)
    }
    return t?.valor || 0
  }
          ...
          {!editando && (
            <div>
              ...
              <div style={{ background: 'white', borderRadius: '12px', border: '1px solid #eef2f7', overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead style={{ background: TEAL_LIGHT }}>
                    <tr>{['Fecha', 'Hora', 'Cliente', 'Duración', 'Modalidad', 'Sede', 'Estado', 'Honorarios', ''].map(h => <th key={h} style={thS}>{h}</th>)}</tr>
                  </thead>
                  <tbody>
                    {clases.length === 0 && <tr><td colSpan={9} style={{ padding: '24px', textAlign: 'center', color: '#aaa' }}>Sin clases este mes</td></tr>}
                    {clases.map((c, i) => {
                      ...
                      return (
                        <tr key={c.id} style={{ borderTop: '1px solid #f8fafc', background: c.revision_pendiente ? '#fff7ed' : i % 2 === 0 ? 'white' : '#fafbfc' }}>
                          ...
                          <td style={{ ...tdS, textTransform: 'capitalize' }}>{c.modalidad || '—'}</td>
          ...
          <div style={{ background: 'white', borderRadius: '16px', width: '90%', maxWidth: '480px', overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.2)' }}>
            ...
            <div style={{ padding: '20px 22px' }}>
              <div style={{ background: TEAL_LIGHT, borderRadius: '10px', padding: '12px 14px', marginBottom: '16px', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', fontSize: '13px' }}>
                ...
                <div><span style={{ color: '#999' }}>Modalidad: </span><strong style={{ textTransform: 'capitalize' }}>{claseModal.modalidad || '—'}</strong></div>
                <div>
              ...
              <div style={{ marginBottom: '16px' }}>
                ...
                {claseModal.observaciones
                  ? <div style={{ background: '#fafbfc', border: '1px solid #e2e8f0', borderRadius: '8px', padding: '10px 12px', fontSize: '13px', color: '#333', lineHeight: '1.5', whiteSpace: 'pre-wrap' }}>{claseModal.observaciones}</div>
                  : <p style={{ color: '#aaa', fontSize: '13px', margin: 0, fontStyle: 'italic' }}>Sin resumen</p>
                }
              </div>
