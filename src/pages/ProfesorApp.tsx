...
function badgeEstado(estado: string, revisionPendiente?: boolean, esTaller?: boolean) {
  ...
  switch (estado) {
    ...
    case 'programada': return { label: 'Sin confirmar', bg: '#f1f5f9', color: '#94a3b8' }
...
clasesDadas.forEach(c => {
  ...
  function verCuentaCobro() {
    generarDocCuentaCobro().getBlob((blob: Blob) => {
 ...
 async function compartirCuentaCobro() {
  ...
  <div style={{ display:'flex', alignItems:'center', gap:'8px' }}>
    ...
    <button onClick={() => supabase.auth.signOut()}
      style={{ background:'rgba(255,255,255,0.15)', border:'none', color:'rgba(255,255,255,0.85)', padding:'5px 11px', borderRadius:'20px', cursor:'pointer', fontSize:'11px', fontWeight:'700', fontFamily:'inherit' }}>
      Salir
    </button>
  </div>
...
{!mostrarFecha && !c.esTaller && !c.es_cortesia && c.numero_proyectado && c.contratos?.total_clases && (
  <span style={{ fontSize:'12px', fontWeight:'800', color:'#b45309', background:'#fffbeb', padding:'2px 8px', borderRadius:'10px', whiteSpace:'nowrap' }}>
   ~clase {c.contratos?.conteo_whatsapp !== null && c.contratos?.conteo_whatsapp !== undefined ? c.contratos.conteo_whatsapp : c.numero_proyectado}/{c.contratos.total_clases}
  </span>
)}
